...
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, weight_ce=1, weight_dice=1, weight_bd=1, ignore_label=None, 
        ...
        self.dc = dice_class(weight=dice_weight, apply_nonlin=softmax_helper_dim1, **soft_dice_kwargs)
        self.bd = Boundary_loss()

    def forward(self, net_output: torch.Tensor, target: torch.Tensor, use_bd=False):
        """
        ...
        if use_bd:
            bd_loss = self.bd(net_output, target_dice) if self.weight_bd != 0 else 0
